-- MAJOR MIGRATION: Per-Dog Subscriptions + Discount Codes System
-- This migration transforms the subscription system from per-user to per-dog

-- ============================================
-- PART 1: BACKUP EXISTING DATA
-- ============================================

-- Backup existing subscriptions table
CREATE TABLE IF NOT EXISTS subscriptions_backup_old AS
SELECT * FROM subscriptions;

-- ============================================
-- PART 2: UPDATE SUBSCRIPTIONS TABLE
-- ============================================

-- Add dog_id column to subscriptions (per-dog subscriptions)
ALTER TABLE subscriptions
ADD COLUMN IF NOT EXISTS dog_id UUID REFERENCES dogs(id) ON DELETE CASCADE;

-- Add index for performance
CREATE INDEX IF NOT EXISTS idx_subscriptions_dog_id ON subscriptions(dog_id);

-- Update existing subscriptions to link to first dog of user (migration helper)
-- In production, you'd manually assign these properly
UPDATE subscriptions s
SET dog_id = (
  SELECT id FROM dogs WHERE owner_id = s.user_id LIMIT 1
)
WHERE dog_id IS NULL;

-- ============================================
-- PART 3: CREATE DISCOUNT CODES TABLE
-- ============================================
...
CREATE TABLE IF NOT EXISTS discount_codes (
  ...
  -- Applicability
  applies_to VARCHAR(50)[] DEFAULT ARRAY['subscription', 'extra_days', 'assessment'], -- What it can be used for
...
-- ============================================
-- PART 4: CREATE DISCOUNT CODE USAGE TABLE
-- ============================================
...
CREATE TABLE IF NOT EXISTS discount_code_usage (
  ...
  -- Usage details
  used_for VARCHAR(50), -- 'subscription', 'extra_days', 'assessment'
...
-- ============================================
-- PART 5: CREATE EXTRA DAYS PURCHASES TABLE
-- ============================================
...
-- ============================================
-- PART 6: UPDATE SUBSCRIPTIONS TO TRACK EXTRA DAYS
-- ============================================
...
-- ============================================
-- PART 9: INSERT SAMPLE DISCOUNT CODES
-- ============================================

-- Insert some example discount codes for testing
INSERT INTO discount_codes (code, description, discount_type, discount_value, valid_until, max_uses, applies_to, is_active)
VALUES
  ('WELCOME10', '10% off first subscription', 'percentage', 10.00, NOW() + INTERVAL '6 months', NULL, ARRAY['subscription'], true),
  ('SUMMER2025', '15% off any purchase', 'percentage', 15.00, '2025-09-01', NULL, ARRAY['subscription', 'extra_days', 'assessment'], true),
  ('EXTRA5', '£5 off extra days', 'fixed_amount', 5.00, NULL, NULL, ARRAY['extra_days'], true)
...
COMMENT ON COLUMN subscriptions.dog_id IS 'Links subscription to specific dog (one subscription per dog)';
