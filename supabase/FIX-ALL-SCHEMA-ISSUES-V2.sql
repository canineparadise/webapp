...
-- Drop existing subscription_tiers table if it exists (so we can recreate with correct schema)
DROP TABLE IF EXISTS subscription_tiers CASCADE;

-- Create subscription_tiers table with correct schema
...
-- Make tier column nullable since we're using tier_id (skip if already nullable)
DO $$
BEGIN
