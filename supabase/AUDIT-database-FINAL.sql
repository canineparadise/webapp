-- ============================================
-- COMPLETE DATABASE AUDIT - FINAL VERSION
-- ============================================
-- Fixed: Removed status column check (doesn't exist in subscriptions)
...
SELECT
  ...
  CASE
    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'assessment_slots') THEN '✅ assessment_slots'
    ELSE '❌ assessment_slots - MISSING'
  END as assessment_slots_status,
  CASE
    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'assessment_bookings') THEN '✅ assessment_bookings'
    ELSE '❌ assessment_bookings - MISSING'
  END as assessment_bookings_status;
...
UNION ALL
SELECT 'assessment_slots', COUNT(*) FROM assessment_slots
UNION ALL
SELECT 'assessment_bookings', COUNT(*) FROM assessment_bookings
ORDER BY table_name;
...
-- 11. Check for subscriptions without owners
SELECT
  s.id,
  s.user_id,
  'Subscription has invalid user_id' as issue
FROM subscriptions s
LEFT JOIN profiles p ON s.user_id = p.id
WHERE p.id IS NULL;

-- 12. Summary counts for dashboard
SELECT
...
SELECT
  ...
  COUNT(*)
FROM profiles
WHERE role = 'user' AND approval_status = 'pending'
UNION ALL
SELECT
  'Total Dogs',
  COUNT(*)
...
-- 13. Subscription health check
SELECT
  'Subscriptions with no days remaining' as issue,
  COUNT(*) as count
FROM subscriptions
WHERE days_remaining = 0 AND is_active = true
UNION ALL
SELECT
  'Subscriptions past end date',
  COUNT(*)
FROM subscriptions
WHERE end_date < CURRENT_DATE AND is_active = true
UNION ALL
SELECT
  'Subscriptions with cancellation requested',
  COUNT(*)
FROM subscriptions
WHERE cancellation_requested = true
UNION ALL
SELECT
  'Subscriptions with auto-renew enabled',
  COUNT(*)
FROM subscriptions
WHERE auto_renew = true AND is_active = true;

-- 14. Check subscriptions table structure
