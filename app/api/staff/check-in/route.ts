...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('[Check-in API] Updating table:', tableName, 'bookingId:', bookingId)

    // Perform the check-in update using admin client (bypasses RLS)
    ...
    console.log('[Check-in API] Update result:', { data, error })
    ...
    if (!data || data.length === 0) {
      console.error('[Check-in API] No rows updated - booking not found:', bookingId)
      return NextResponse.json({ error: 'No booking found with that ID' }, { status: 404 })
    }

    console.log('[Check-in API] Success - updated booking:', data[0].id)
